    ...
    const ClientForm: React.FC<ClientFormProps> = ({ client, onSubmit, onClose, categories, tags }) => {
      ...
      const { control, handleSubmit, formState: { errors }, setValue, watch } = useForm<CustomFormData>({
        defaultValues: client || {
          ...
          contactEmail: '',
          contactPhone: '',
          hasInvoiceRegistration: false, // 新規追加
          registrationNumber: '',
          categoryId: null,
          tagIds: [],
          status: 'active',
      ...
      useEffect(() => {
        ...
        } else if (!client && watch('companyName') === 'none') {
          setValue('companyName', '');
        }
      }, [isPersonal, setValue, watch, client]);
            ...
            <div>
              {!isPersonal ? (
                <Input
                  {...field}
                  type="text"
                  placeholder="会社名(個人の場合はnoneと入力)"
                  value={field.value || ''}
                  disabled={isPersonal}
                />
              ) : (
                <input type="hidden" {...field} value="none" />
              )}
